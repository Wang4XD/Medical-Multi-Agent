...
class HuatuoChatbot():
    ...
    def init_components(self):
        ...
        if 'huatuogpt-vision-7b' in d.lower():
            ...
            model, loading_info = LlavaQwen2ForCausalLM.from_pretrained(self.model_dir,
            ...
            model, loading_info = LlavaLlamaForCausalLM.from_pretrained(self.model_dir,
                                                                        init_vision_encoder_from_ckpt=True,
                                                                        output_loading_info=True,
                                                                        torch_dtype=torch.bfloat16)
            missing_keys = loading_info['missing_keys']  # keys exists in model architecture but does not exist in ckpt
    ...
    def preprocess_huatuo(self, convs: list, return_tensors) -> list:  # tokenize and concat the coversations
        ...
        for ind in range(round_num):
            ...
            cur_input_ids = self.tokenizer(g, add_special_tokens=False, truncation=True, return_tensors='pt').input_ids[
                0]
            input_ids = torch.cat([input_ids, cur_input_ids])
    ...
    def chat(self, text: str, images: list[str] = None, ):
        ...
        generation_kwargs = dict(inputs=input_ids, images=image_tensors.to(
            dtype=torch.bfloat16) if image_tensors is not None else image_tensors, streamer=streamer, use_cache=True,
                                 **self.gen_kwargs)
